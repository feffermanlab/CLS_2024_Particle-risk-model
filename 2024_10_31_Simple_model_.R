...
parms <-data.frame(s=0.05,a=10, d=0.3,bet_bar = 1,bet_hat =100 )
...
#{r}
Small_bld_data_clean <- Small_bld_output_v3%>% pivot_longer(cols = !time,
                                                  names_to = c("State", "Room"),
                                                  names_pattern = "(P|N_x)(\\d+)",
                                                  values_to = "Number")


Small_bld_data_clean %>% filter(State == "P") %>% 
  group_by(State,Room)%>% 
  ggplot( aes(x=time, y = Number, group =Room, color = Room))+
  geom_line()+theme_classic()+
  labs(x = "Time (hours)", y= "Number")+ggtitle("Number of Infectious particles")

Small_bld_data_clean %>% filter(State == "N_x") %>% 
  group_by(State,Room)%>% 
  ggplot( aes(x=time, y = Number, group =Room, color = Room))+
  geom_line()+theme_classic()+
  labs(x = "Time (hours)", y= "Number")+ggtitle("Number of People in rooms")

Small_bld_data_clean %>% filter(State == "N_x") %>% ungroup() %>% group_by(time) %>% summarise(Total_prop=sum(Number))%>%
...
ratio_data <- temp_data %>% pivot_wider(names_from = c(State),values_from = c(Number)) %>% 
  group_by(time, Room)%>%mutate(s =(parms$s),a = parms$a,bet_bar = parms$bet_bar,bet_hat= parms$bet_hat, Ib_prop = Small_bld_setup$Ib_prop[1],shed = s*N_x*Ib_prop*C_x*lam , Denominator =(a*P), ratio = Numerator/Denominator ) 
