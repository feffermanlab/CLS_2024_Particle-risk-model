...
```{r initialize building, include=FALSE}
...
# Church_setup <- Bld_setup_func(Community_output = Community_output,day = day,delt = delt,N_rooms =length(small_bld_3_rooms_C))
# Church_setup
...
Small_bld_setup
r1_pop <-(Small_bld_setup$S[1]+Small_bld_setup$I[1]+Small_bld_setup$R[1])*Adj_Max_Building_Capacity_small_bld
r2_pop <-(Small_bld_setup$S[2]+Small_bld_setup$I[2]+Small_bld_setup$R[2])*Adj_Max_Building_Capacity_small_bld
r3_pop<- (Small_bld_setup$S[3]+Small_bld_setup$I[3]+Small_bld_setup$R[3])*Adj_Max_Building_Capacity_small_bld
r1_pop+r2_pop+r3_pop
```
...
```{r function for model, include=FALSE}
...
Small_bld_Init_conds_v2 <-c(S=Small_bld_setup$S, I = Small_bld_setup$I, R = Small_bld_setup$R, P = Small_bld_setup$P,N_x =Small_bld_setup$N_x)
...
data_clean <- Small_bld_output_v2%>% pivot_longer(cols = !time,
                                                   ...
                                                   names_pattern = "([A-Za-z]+)(\\d+)",
                                                   values_to = "Number")
data_ratios <- data_clean %>% pivot_wider(names_from = c(State),values_from = c(Number)) %>% group_by(time,Room)  %>% 
  mutate(N_x = (S+I+R)*Adj_Max_Building_Capacity_small_bld, ratio = x/N_x) #%>% group_by(time) %>% summarise(Total_prop=sum(N_x), x =sum(x))
# %>% group_by(time,Room) %>% 
#   mutate(K_x = ((parms$s)*I)/((parms$a)*N_x), prop_to_K = P/K_x)
# Church_P_x_K_x_plot<-ggplot(Church_data_ratios,aes(x =time, y =prop_to_K,group= Room,color=Room))+geom_line()+labs(title = "Church - Risk - 31 rooms")
ggplot(data_ratios,aes(x=time, color = Room))+geom_line(aes(x = time, y = x/N_x))#+geom_line(aes(x = time, y = N_x))
...
p6_v2 <- data_clean %>% filter(State == "S"| State == "I" | State == "R") %>% ungroup() %>% group_by(time, Room) %>% summarise(Total_prop=sum(Number)) %>%
  ggplot(aes(x=time, y=Total_prop, color=Room))+geom_line()+
  labs(x = "Time (hours)", y= "Proportion of individuals")+ggtitle("Proportion of individuals in rooms within a building")

p7_v2 <- data_clean %>% filter(State == "S"| State == "I" | State == "R") %>% ungroup() %>% group_by(time) %>% summarise(Total_prop=sum(Number)) %>%
  ggplot(aes(x=time, y=Total_prop))+geom_line()+
  labs(x = "Time (hours)", y= "Proportion of individuals")+ggtitle("Proportion of individuals in the building")


p8_v2 <- data_clean %>% filter(State == "x") %>% ungroup() %>% group_by(time, Room) %>% summarise(Total_prop=sum(Number))%>% arrange(desc(Total_prop))%>%
  ggplot(aes(x=time, y=Total_prop, color=Room))+geom_line()+
  labs(x = "Time (hours)", y= "Number of individuals")+ggtitle("Number of individuals in the building")
